<?php
    ...
    // Get job ID from query string and validate it
    $jobId = $_GET['id'] ?? null;
    ...
    $jobData = null;
    $stmt = $conn->prepare("SELECT id_jobtitles, id_industry, job_title, created_by, date_created, last_updated FROM rm_jobtitles WHERE id_jobtitles = ?");
    ...
    // Prepare formatted dates for display
    $currentFormattedDate = date('F j, Y');
    $dateCreatedFormatted = date('F j, Y', strtotime($jobData['date_created']));
    $lastUpdatedFormatted = date('F j, Y', strtotime($jobData['last_updated']));
...
<body>
    ...
    <div class="main-wrapper">
        ...
        <main class="main-content bodybg ">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
                <h4 style="margin: 0;" class="titleupdate">Job Title Update</h4>
            ...
            <div style="max-width: auto; margin: 20px auto; padding: 20px; border: 1px solid #ccc; border-radius: 10px; background-color: #FFFFFF;">
                <form class="form-container" method="post" id="editJobForm">
                    <input type="hidden" name="jobId" value="<?= htmlspecialchars($jobData['id_jobtitles']) ?>">

                    <div class="mb-3">
                        <label for="jobname" class="form-label">Job Name</label>
                        <input type="text" class="form-control" id="editjobname" name="editjobname"  value="<?= htmlspecialchars($jobData['job_title']) ?>">
                    </div>

                    <div class="mb-3">
                        <label for="industryCategory" class="form-label">Industry Category</label>
                        <div class="select-wrapper position-relative">
                            <select class="form-control" id="editjobIndustryCategory" name="editjobIndustryCategory">
                                <option value="">Please Select</option>
                                <?php foreach ($industries as $industry): ?>
                                    <option value="<?= $industry['id_industry'] ?>" <?= ($industry['id_industry'] == $jobData['id_industry']) ? 'selected' : '' ?>>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="editjobCreatedBy" name="editjobCreatedBy" value="<?= htmlspecialchars($jobData['created_by']) ?>" readonly>
                    </div>

                    <div class="mb-3">
                        <label for="dateCreated" class="form-label">Date Created</label>
                        <input type="text" class="form-control" id="editjobDateCreated" name="editjobDateCreated" value="<?= $dateCreatedFormatted ?>" readonly>
                    </div>

                    <div class="mb-4">
                        <label for="lastUpdated" class="form-label">Last Updated</label>
                        <input type="text" class="form-control" id="editjobLastUpdated" name="editjobLastUpdated" value="<?= $lastUpdatedFormatted ?>" readonly>
                    </div>

                    <button type="submit"  id="triggerConfirmationModal" class="addbtn btn btn-primary"> Save Changes </button>
                </form>
            </div>
        </main>
    </div>  
    ...
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.11.3/font/bootstrap-icons.min.css">
    <script src="http://localhost/Resumix/Frontend/Admin/js/jobtitle_edit.js"></script>
