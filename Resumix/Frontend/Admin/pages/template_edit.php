<?php
    ...
    if ($templateId > 0) {
        $stmt = $conn->prepare("SELECT * FROM rm_templates WHERE id_template = ?");
        ...
        if ($template) {
            $template['date_created'] = date("F j, Y", strtotime($template['date_created']));
            $template['last_updated'] = date("F j, Y", strtotime($template['last_updated']));
        }
    }

        $industries = [];

        $industryStmt = $conn->prepare("SELECT id_industry, industry_name FROM rm_industry");
        $industryStmt->execute();
        $industryResult = $industryStmt->get_result();
...
<body>
    ...
    <div class="main-wrapper">
        ...
        <main class="main-content bodybg ">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
                <h4 style="margin: 0;" class="titleupdate">Resume Template Update</h4>
            ...
            <div style="max-width: auto; margin: 20px auto; padding: 20px; border: 1px solid #ccc; border-radius: 10px; background-color: #FFFFFF;">
                <form method="POST" enctype="multipart/form-data" id="edittemplateForm">
                            ...
                            <div style="border: 1px solid #ddd; border-radius: 10px; padding: 15px; display: inline-block; height: 250px; width: 190px; overflow: hidden;">
                                <?php
                                    $imageData = !empty($template['image_template']) 
                                        ...
                                        : 'http://localhost/Resumix/Images/whitetemp.png';
                                ?>
                                <img src="<?= $imageData ?>" alt="Template Preview" class="img-thumbnail" id="Tempimage" style="width: 190px; height: 217px; border-radius: 5px;">
                            </div>

                            <div class="custom-file-container">
                                <label for="edittemplateImage" class="custom-file-button">Choose File</label>
                                <span class="file-name" id="fileName">
                                    <?= !empty($template['image_template']) ? 'existing_image.jpg' : 'No file chosen' ?>
                                </span>
                                <input type="file" id="edittemplateImage" name="edittemplateImage" onchange="addFileImage()" style="display: none;">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="edittemplateName" name="edittemplateName" value="<?= htmlspecialchars($template['template_name']) ?>">
                    </div>

                    <div class="mb-3">
                        <label for="description" class="form-label">Description</label>
                        <textarea class="form-control" id="edittemplateDescription" name="edittemplateDescription" rows="3"><?= htmlspecialchars_decode($template['description']) ?></textarea>
                    </div>

                    <div class="mb-3">
                        <label for="industryCategory" class="form-label">Industry Category</label>
                        <div class="select-wrapper position-relative">
                            <select class="form-control" id="edittemplateIndustryCategory" name="edittemplateIndustryCategory">
                                <option value="">Please Select</option>
                                <?php foreach ($industries as $industry): ?>
                                    <option value="<?= $industry['id_industry'] ?>" 
                                        <?= isset($template['id_industry']) && $template['id_industry'] == $industry['id_industry'] ? 'selected' : '' ?>>
                    ...
                    <div class="row mb-3">
                        <div class="col-md-6">
                            ...
                            <input type="number" class="form-control" id="edittemplateTimesUsed" name="edittemplateTimesUsed" value="<?= (int)$template['times_used'] ?>" readonly>
                        </div>
                        <div class="col-md-6 mt-3 mt-md-0">
                            <label for="downloads" class="form-label">Downloads</label>
                            <input type="number" class="form-control" id="edittemplateDownloads" name="edittemplateDownloads" value="<?= (int)$template['downloads'] ?>" readonly> 
                        </div>
                    </div>

                    <div class="mb-3">
                        <div class="custom-file-container">
                            <label for="edittemplateFile" class="custom-file-button">Choose File</label> 
                                <span class="file-name" id="fileTypeName">
                                    <?= !empty($template['file_template']) ? 'existing_template.docx' : 'No file chosen' ?>
                                </span>
                            <input type="file" id="edittemplateFile" name="edittemplateFile" onchange="addTemplateFile()" style="display: none;">
                        </div>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="edittemplateCreatedBy" name="edittemplateCreatedBy" value="<?= htmlspecialchars($template['created_by']) ?>" readonly>
                    </div>

                    <div class="mb-3">
                        <label for="dateCreated" class="form-label">Date Created</label>
                        <input type="text" class="form-control" id="edittemplateDateCreated" name="edittemplateDateCreated" value="<?= htmlspecialchars($template['date_created']) ?>" readonly>
                    </div>

                    <div class="mb-4">
                        <label for="lastUpdated" class="form-label">Last Updated</label>
                        <input type="text" class="form-control" id="edittemplateLastUpdated" name="edittemplateLastUpdated" value="<?= htmlspecialchars($template['last_updated']) ?>" readonly>
                    </div>

                    <div class="d-grid">
                        <button type="submit" class="addbtn btn btn-primary btn-lg" id="saveChangesBtn">Save Changes</button>
                    </div>
                </form>
            </div>
        </main>
    ...
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.11.3/font/bootstrap-icons.min.css">
    <script src="http://localhost/Resumix/Frontend/Admin/js/template_edit.js"></script>
