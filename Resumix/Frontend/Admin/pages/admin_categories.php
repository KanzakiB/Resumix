<?php
    ...
    if (!empty($search)) {
        ...
        $searchCondition = "WHERE industry_name LIKE '%$searchSafe%'";
    }

    // Total count for pagination
    $totalQuery = "SELECT COUNT(*) as total FROM rm_industry $searchCondition";
    $totalResult = $conn->query($totalQuery);
    $totalRow = $totalResult->fetch_assoc();
    $total_industries = $totalRow['total'];
    $total_pages = ceil($total_industries / $usersPerPage);

    // Fetch industries
    $industries = [];

    $sql = "SELECT id_industry, industry_name FROM rm_industry $searchCondition ORDER BY id_industry ASC LIMIT $offset, $usersPerPage";
...
<head>
    ...
    <title>Industry Categories</title>
    ...
    <link href="http://localhost/Resumix/Frontend/Admin/css/admin_categories.css" rel="stylesheet">
...
<body>
    ...
    <div class="main-wrapper">
        <header class="header-main">
            <div class="d-flex align-items-center">
                ...
                <h1 class="header-title d-none d-md-block titleheader">Industry Categories</h1>
            </div>
        ...
        <main class="main-content bodybg ">
            <div class="container mt-4">
                    <div class="d-flex justify-content-between align-items-center mb-1">
                        ...
                        <a href="admin_categories.php" style="padding-bottom: 8px; margin-right: 20px; text-decoration: none; color: #00085C; font-weight: bold; border-bottom: 2px solid #00085C;">Industry Categories</a>
                        <a href="admin_job.php" style="padding-bottom: 8px; text-decoration: none; color: #333;">Job Titles</a>
                        </div>
                    </div>
                    ...
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        ...
                        <!-- Add button -->
                        <div>
                            <button class="addbtn btn btn-outline-secondary" onclick="location.href='industry_add.php'">
                                ...
                                <tbody>
                                    <?php if (!empty($industries)): ?>
                                        <?php foreach ($industries as $industry): ?>
                                            <tr>
                                                <td><?= htmlspecialchars($industry['id_industry']) ?></td>
                                                <td><?= htmlspecialchars($industry['industry_name']) ?></td>
                                                <td class="action-icons">
                                                    <a href="javascript:void(0);" data-industry-name="<?= htmlspecialchars($industry['industry_name']) ?>" class="list" data-industry-id="<?= $industry['id_industry'] ?>" title="View Jobs"  style="text-decoration: none; color: inherit; background-color: transparent;">
                                                        <i class="fa-solid fa-list-ul listbtn"></i>
                                                    </a>
                                                    <a href="industry_edit.php?id=<?= $industry['id_industry'] ?>" title="Edit"  style="text-decoration: none; color: inherit; background-color: transparent;">
                                                        <i class="fa-regular fa-pen-to-square editbtn"></i>
                                                    </a>
                                                    <button class="deletebutton text-danger" data-industry-id="<?= $industry['id_industry'] ?>" title="Delete">
                                    ...
                                    <?php else: ?>
                                        <tr>
                                            <td colspan="3" class="text-center text-muted">No industry found...</td>
                                        </tr>
                                    <?php endif; ?>
                                </tbody>
                            </table>
    ...
    <!-- View Job Modal -->
    <div class="modal fade" id="jobTitlesModal" tabindex="-1" aria-labelledby="jobTitlesModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content p-3">
                <div class="modal-header">
                    <h5 class="modal-title" id="jobTitlesModalLabel">Industry Name</h5>
                </div>
                <div class="modal-body">
                    <p class="fw-bold mb-3">Job Title List</p>
                    <div class="job-title-list-container">
                        <ul id="jobTitlesList" class="list-unstyled mb-0"></ul> 
                    </div>
                </div>
    ...
    <script src="http://localhost/Resumix/Frontend/Admin/js/industry_category.js"></script>
